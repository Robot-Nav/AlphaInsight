...
## 1. 项目含义
在私募股权投资和二级市场投研中，分析师通常需要耗费数周时间阅读海量非结构化长文本（如上百页的招股说明书、年度财报、高管电话会议纪要等）。传统的 OCR 或简单的 NLP 抽取技术无法理解复杂的财务逻辑（例如“应收账款异常增长隐含的坏账风险”）。
**AlphaInsight** 通过引入大语言模型（LLM）的超长上下文处理能力与多智能体（Multi-Agent）协作机制，打破了这一困境。本项目基于 LangGraph 构建非线性尽调工作流，旨在利用 AI 自动化完成从数据检索、深度财务分析到风控排查的全链路尽调工作，输出带有精准内容溯源的深度尽调报告。
...
* **长链财务分析**：**财务分析 Agent** 执行长链推理 (Long-chain Reasoning)，不仅仅是提取数据，还会深度结合财务模型挖掘利润下滑或异常增长背后的深层经营原因。
* **对抗性风控审查（亮点）**：**风控做空 Agent** 拥有独立的 System Prompt，专门扮演“挑刺者”的角色。它会与财务分析 Agent 展开多轮辩论（例如：质疑营收增长下的供应商集中度问题与存货周转矛盾），迫使系统自我反思，避免分析视角的盲目乐观。
* **汇总与生成报告**：经过 3-5 轮的多智能体深度辩论后，**主笔 Agent** 会汇总最终共识与分歧，并调用 Python 代码解释器自动绘制财务走势图表。最终输出一份长达 20 页、且附带精确溯源（直接引用至 PDF 具体段落）的深度尽调报告。

## 3. 算法原理
* **非线性状态机图工作流 (LangGraph)**：打破传统的线性 Chain 模式，利用图结构（Nodes 代表 Agent 或函数，Edges 代表条件路由）管理多智能体间的交互，支持循环、条件分支和多 Agent 的多轮对话管理。
* **多智能体对抗辩论机制 (Adversarial Agent Debate)**：借鉴生成式对抗网络 (GAN) 和博弈论思想。引入“财务看多”与“风控做空”的双视角对抗机制，通过多轮交叉质询（Critique）和自我反思修正（Refine），收敛出更严谨客观的尽调结论。
* **检索增强生成 (RAG)**：对研报和 SEC 文件等非结构化数据进行文本分块 (Chunking) 与向量化 (Embedding)，通过高维向量检索匹配最相关的文本片段，解决长文本处理过程中的大模型幻觉问题，并实现精确到段落的引用溯源。
* **长链推理 (Long-chain Reasoning / CoT)**：通过系统级 Prompt 工程引导大模型按部就班地进行逻辑推理，将复杂的财务分析问题拆解为多个具有先后顺序的子问题。

## 4. 算法公式
项目运行中涉及的核心数学算法与财务分析理论模型：

**1. 向量检索的余弦相似度计算 (Cosine Similarity)**
在 RAG 知识库检索模块，用于衡量用户查询向量 $Q$ 与文档片段向量 $D$ 之间的语义相似度：
$$ \text{Similarity}(Q, D) = \frac{Q \cdot D}{||Q|| \times ||D||} = \frac{\sum_{i=1}^{n} Q_i D_i}{\sqrt{\sum_{i=1}^{n} Q_i^2} \sqrt{\sum_{i=1}^{n} D_i^2}} $$
